...
func TestMutexSemaphore_TryAcquire(t *testing.T) {
	tests := []struct {
		name        string
		capacity    int64
		permits     int64
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sem := NewMutexSemaphore(tt.capacity)
...
func TestMutexSemaphore_Release(t *testing.T) {
	tests := []struct {
		name           string
		capacity       int64
		acquireFirst   int64
	...
	}{
		{
			...
			releasePermits: 2,
			wantPermits:    3,
			description:    "should restore permits after release",
		},
		{
			...
			releasePermits: 1,
			wantPermits:    3,
			description:    "should partially restore permits",
		},
		{
			...
			releasePermits: 5,
			wantPermits:    3,
			description:    "should cap at max capacity when releasing more",
		},
		{
			...
			releasePermits: 2,
			wantPermits:    3,
			description:    "should cap at max capacity when releasing without acquire",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			success := sem.TryAcquire(tt.wantPermits)
			assert.True(t, success, tt.description)
		})
	}
}

func TestMutexSemaphore_Acquire(t *testing.T) {
	tests := []struct {
		name        string
		capacity    int64
		permits     int64
